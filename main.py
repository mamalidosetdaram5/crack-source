...
async def handle_user_message(event):
    if event.sender_id != admin_user_id and (load_state().get('self') or load_state().get('chats', {}).get(str(event.chat_id), {}).get('self')):
        message = event.raw_text
        reply = fast_replies.get(message.lower())
        if reply:
            await event.reply(reply)

# Unified feature pack: command controls and passive behaviors.
@client.on(events.NewMessage)
async def handle_self_features(event):
    await feature_handler(event)
